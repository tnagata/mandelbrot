...
*/
    ...
    let pixel_ptr = SendPtr(pixels.as_mut_ptr());
    let bands_ref = &bands;

    crossbeam::scope(|scope| {
    for _ in 0..threads {
        scope.spawn(move |_| {
            while let Some((start, end)) = bands_ref.next() {
                let band = unsafe { std::slice::from_raw_parts_mut(pixel_ptr.as_ptr().add(start), end - start) };   // ← safe に &mut を作れる
